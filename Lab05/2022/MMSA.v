...
always@(posedge clk) begin
  ccnt_d[0] <= ccnt;
  for (i = 0; i <= 14; i = i + 1) ccnt_d[i+1] <= ccnt_d[i];
end
...
always@(posedge clk) begin
  ...
  case(state)
    Wait, Process: if (!stop_p) m_valid[0] <= 1'b1;
  endcase
end
...
always@(*) begin
  ...
  case (state)
    ...
    Output : begin
      for (i = 1; i <= 15; i = i + 1)  m_A[i] <= {mi, ccnt_d[i]};
    end
  endcase
end
...
always@(posedge clk) begin
  case (state)
    ...
    Wait :
      begin
        ccnt <= ccnt_next;
        for (i = 0; i <= 15; i = i + 1) w_mem[i][0] <= w_Q[i];
      end
    Process :
      begin
        ccnt <= ccnt_next;
        pcnt <= pcnt_add_1;
        if (!stop_p) for (i = 0; i <= 15; i = i + 1) w_mem[i][pcnt_add_1] <= w_Q[i];
      end
    Output :
      begin
        ccnt <= 4'b0;
